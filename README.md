# WDD 360 Final Portfolio
All of my work from this semester organized for the final submission.

### Individual Repository for Non-Team Work

* [wdd360-repository](https://github.com/Larson04/wdd360-repository)

### Sleep Outside Team Repositories

* [Sleep Outside Client Repository](https://github.com/Larson04/wdd360-sleepoutside)
* [Sleep Outside Server Repository](https://github.com/Larson04/wdd360-sleepoutsideSERVER)
* [Sleep Outside Trello Board](https://trello.com/b/TWxmuI3T/wdd360-sleep-outside)

### Final Project: Rate My Class Links and Test Users

* [Rate My Class Live Site](https://rate-myclass.netlify.app/)
* [Rate My Class Client Repository](https://github.com/21sarchibald/rate-my-class-client)
* [Rate My Class Server Repository](https://github.com/21sarchibald/rate-my-class-server)

    * Here is the [Rate My Class Trello Board](https://trello.com/b/QRX8B3h7/rate-my-class) in case you would like to view it.

#### Test Users

* Alice Student
   * Email: alice.student@test.com
   * Password: Test123!
   * Role: student
* Ava Admin
   * Email: ava.admin@test.com
   * Password: Test456!
   * Role: admin 

